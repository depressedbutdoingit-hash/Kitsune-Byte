...
class MonetizationEngine {
  ...
  static const Map<SubscriptionTier, TierConfig> tierConfigs = {
    ...
    SubscriptionTier.pro: TierConfig(
      ...
      maxTeamSeats: 3,
      features: {'visual_builder', 'terminal', 'local_ai', 'full_swarm', 'deployments', 'vps_deployment', 'app_store_agent', 'custom_agents', 'team_collab', 'priority_support'},
    ),
    SubscriptionTier.power: TierConfig(
      ...
      maxTeamSeats: 10,
      features: {'visual_builder', 'terminal', 'local_ai', 'full_swarm', 'deployments', 'vps_deployment', 'custom_vps', 'app_store_agent', 'custom_agents', 'team_collab', 'priority_models', 'white_label', 'dedicated_support'},
    ),
  };
  ...
  Future<UsageResult> canUseFeature({
    ...
    if (!config.features.contains(feature)) {
      return UsageResult.denied('Upgrade to ' + config.name + ' to use ' + feature);
    }
    if (!config.allowedModelTiers.contains(modelTier)) {
      return UsageResult.denied(modelTier.toString() + ' models require higher plan');
    }
    final remaining = await _getRemainingCredits(userId);
    if (remaining < estimatedCreditCost) {
      return UsageResult.denied('Out of AI credits. ' + remaining.toStringAsFixed(0) + ' remaining.', suggestion: 'upgrade_or_topup');
    }
